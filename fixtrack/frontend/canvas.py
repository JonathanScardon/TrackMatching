...
class VideoCanvas(CanvasBase):
    ...
    '''
    def __init__(self, parent, fname_video=None, fname_track=None, fname_candidates=None, is_left=False, **kwargs):
        ...
        self.fname_candidates = fname_candidates
        ...
        if is_left:
            self.candidates = Candidates(self.fname_candidates)
            self.visuals["tracks"] = TrackCollectionVisual(self.tracks, parent=self, candidates=self.candidates, is_left=True)
        else:
    ...
    def on_frame_change(self, frame_num=None, refresh_data = True):
        ...
        # print("on_frame_change from VideoCanvas, frame " + str(frame_num))
